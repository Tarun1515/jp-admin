...
The Staffroom India app for **administrators**. Angular 22, standalone components,
...
> `npm start` refuses to move to a different port if 4200 is busy. That is
...
- Sign in (there is **no signup** — admin accounts come from `JP.Tools.SeedAdmin`
  or from another administrator, decision 2.9)
- Forgot password, reset password
- Admin shell

Phase 2E lands here, including master-data management — one config-driven
screen for all 17 master tables rather than 17 near-identical ones.

---

## Signing in

Only **admin accounts** work here. All the apps authenticate against the
same SSO API, so a school owner or teacher **can** sign in successfully — valid token, wrong
